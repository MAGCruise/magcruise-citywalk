...
public class TableModelTest {
	...
	public void test() {
		...
		checkpoints.merge(new Checkpoint("aed-1", "aed-1", "A号館のAED", 38.4400, 134.11090, Arrays.asList("waseda"),
				"blue", "category-2", "subcategory-2-1", from, to));
		checkpoints.merge(new Checkpoint("aed-2", "aed-2", "B号館のAED", 38.4400, 134.11090, Arrays.asList("waseda"),
				"blue", "category-2", "subcategory-2-1", from, to));
		checkpoints.merge(new Checkpoint("cafeteria", "食堂", "みんながいく食堂", 38.4400, 134.11090, Arrays.asList("waseda"),
				"red", "category-1", "subcategory-1-1", from, to));
		checkpoints.merge(new Checkpoint("pc-room", "PCルーム", "演習で使うPCルーム", 38.4400, 134.11090, Arrays.asList("waseda"),
				"green", "category-1", "subcategory-1-2", from, to));
		...
		tasks.insert(new Task("cafeteria-selection", Arrays.asList("cafeteria"),
				new SelectionTask("次のうち、理工の学食が発祥の地であるメニューはどれ？", Arrays.asList("豚玉丼", "チキンおろしだれ", "カツカレー", "ポーク焼肉"),
						Arrays.asList(3), 2.0, false)));
		String tid = tasks.getLastInsertId(Task.class).toString();
		tasks.insert(new Task("aed-selection", Arrays.asList("aed-1", "aed-2", "aed-3", "aed-4", "aed-5", "aed-6"),
				new SelectionTask("次のうち、AEDの使い方として、間違っているのはどれ？",
						Arrays.asList("パッドは右胸と左わき腹に貼る", "心電図解析中は体に触らない", "放電ボタンを押す時は、体から離れる", "呼吸が戻ったらパッドを速やかにはずす"),
						Arrays.asList(3), 2.0, false)));
		tasks.insert(new Task("pc-room-sort", Arrays.asList("pc_room"), new SelectionTask("次の4つの部屋を、座席の多い順に並び替えて下さい．",
		...
		activities.insert(new VerifiedActivity("waseda", "ayaki", "cafeteria", 38.4400, 134.11090, tid, 1.0,
